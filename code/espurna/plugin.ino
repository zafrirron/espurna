...
const char POST_REQUEST_TEMPLATE[] PROGMEM =
        ...
        "Content-Length: %d\r\n\r\n"
        "%s\r\n";
const String LoggerHost = PLUGIN_LOG_HOST;      //Logger Server IP
const uint16_t LoggerPort = PLUGIN_LOG_PORT;    //Logger service port
const String LogUri = PLUGIN_LOG_HOST;          //Simple logger service definition
...
void _pluginBroker(const String& topic, int id, unsigned int value) {
    ...
    if (topic.equals(MQTT_TOPIC_BUTTON)) {
        ...
        if (id == 0) {
            if (value == 2) {              //click action
                //do somthing
            } else if (value == 3) {       //double click
                //do somthing
            } else if (value == 4) {       //long click
                //do somthing
            } else if (value == 5) {       //long long
                //dor something
            } else if (value == 6) {       //triple click
            }

        }
        
    } else if (topic.equals(MQTT_TOPIC_RELAY)) {
        DEBUG_MSG_P(PSTR("[PLUGIN] Relay #%d - %s\n"), id, value == 1 ? "ON" : "OFF");
        if (value == 1) {           // Relay1 on actions
            if (id == 0) {          // Relay 1
                //do somthing
            } else if (id == 1) {   //Relay 2
                //do somthing
            } else if (id == 2) {   //Relay 3
                //do somthing
            } else if (id == 3) {   //Relay 4
                //do somthing
            } else if (id == 4) {   //Relay 5
                //do somthing
            }

            //DEBUG_MSG_P(PSTR("[PLUGIN] Relay(d%) action: %s\n"),value == 1 ? "ON" : "OFF");
        }
    }
}
#endif
